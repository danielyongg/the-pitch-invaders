    ...
    <div className="max-w-4xl mx-auto px-8 py-10">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-8">
        <div className="min-w-0">
          <h1 className="font-[var(--font-anybody)] font-bold text-[28px] sm:text-[40px] text-[#e1e2ec] break-words [font-variation-settings:'wdth'_100]">{league.name}</h1>
          <p className="text-[#c3c6d3] mt-1 font-[var(--font-jetbrains)] tracking-wide">
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-[rgba(255,255,255,0.1)] bg-[rgba(25,27,35,0.5)]">
          <h2 className="font-[var(--font-anybody)] font-semibold text-xl text-[#e1e2ec] [font-variation-settings:'wdth'_100]">League Standings</h2>
        </div>

        {!leaderboard?.length ? (
          <div className="text-center py-12 text-[#c3c6d3]">
            <p>No points yet. Start predicting!</p>
          </div>
        ) : (
          <div>
            <div className="grid grid-cols-[32px_1fr_60px] sm:grid-cols-[80px_1fr_100px_80px_80px] gap-2 sm:gap-4 px-4 sm:px-6 py-3 text-xs font-[var(--font-jetbrains)] tracking-widest uppercase text-[#c3c6d3] border-b border-[rgba(255,255,255,0.1)] bg-[rgba(25,27,35,0.3)]">
              <span>#</span>
              <span>Player</span>
            ...
            {leaderboard.map((entry, idx) => {
              ...
              return (
                <div
                  key={entry.user_id}
                  className={`grid grid-cols-[32px_1fr_60px] sm:grid-cols-[80px_1fr_100px_80px_80px] gap-2 sm:gap-4 px-4 sm:px-6 py-4 items-center border-b border-[rgba(255,255,255,0.05)] last:border-0 transition ${
                ...
                >
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    <div className="hidden sm:flex w-8 h-8 rounded-full bg-[#272a32] border border-[rgba(174,198,255,0.2)] items-center justify-center text-sm font-bold text-[#e1e2ec] flex-shrink-0">
                      {entry.username[0].toUpperCase()}
                    </div>
                    <span className={`text-sm font-medium truncate ${isMe ? 'text-[#aec6ff]' : 'text-[#e1e2ec]'}`}>
                      {entry.username}
