-- Swap PEN-match point values: correctly predicting the scoreline stays
-- level (regulation/AET draw, which is why it went to penalties at all) is
-- now worth more (2) than correctly predicting which side wins the shootout
-- (1) — user-requested reweighting, previous order was 2 for shootout
-- winner / 1 for draw.
...
-- Re-score every already-finished PEN match with the corrected weighting so
-- past predictions get the new point values too.
do $$
declare
  m record;
begin
